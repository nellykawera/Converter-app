...
function MassConverter() {
  ...
  const convertMass = () => {
    ...
    if (!isNaN(inputValueFloat)) {
      ...
      if (fromUnit === 'kilogram' && toUnit === 'pound') {
        ...
        convertedMass = inputValueFloat * 1000;
      } else if (fromUnit === 'gram' && toUnit === 'kilogram') {
        convertedMass = inputValueFloat / 1000;
      } else if (fromUnit === 'pound' && toUnit === 'gram') {
        convertedMass = inputValueFloat * 453.592;
  ...
  return (
    <div className="converter-container" style={{ backgroundImage: `url(${massConverterBackgroundImage})`, backgroundSize: 'cover', backgroundRepeat: 'no-repeat', backgroundPosition: 'center', backgroundColor: 'green', padding: '20px', borderRadius: '8px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
          ...
          <div>
            <input type="radio" id="kilogram" name="fromUnit" value="kilogram" checked={fromUnit === 'kilogram'} onChange={() => setFromUnit('kilogram')} />
            <label htmlFor="kilogram" style={{ fontSize: '16px', marginLeft: '5px', color: 'white' }}>Kilogram</label>
          </div>
          <div>
            <input type="radio" id="pound" name="fromUnit" value="pound" checked={fromUnit === 'pound'} onChange={() => setFromUnit('pound')} />
            <label htmlFor="pound" style={{ fontSize: '16px', marginLeft: '5px', color: 'white' }}>Pound</label>
          </div>
          <div>
            <input type="radio" id="gram" name="fromUnit" value="gram" checked={fromUnit === 'gram'} onChange={() => setFromUnit('gram')} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
          ...
          <div>
            <input type="radio" id="toKilogram" name="toUnit" value="kilogram" checked={toUnit === 'kilogram'} onChange={() => setToUnit('kilogram')} />
            <label htmlFor="toKilogram" style={{ fontSize: '16px', marginLeft: '5px', color: 'white' }}>Kilogram</label>
          </div>
          <div>
            <input type="radio" id="toPound" name="toUnit" value="pound" checked={toUnit === 'pound'} onChange={() => setToUnit('pound')} />
            <label htmlFor="toPound" style={{ fontSize: '16px', marginLeft: '5px', color: 'white' }}>Pound</label>
          </div>
          <div>
            <input type="radio" id="toGram" name="toUnit" value="gram" checked={toUnit === 'gram'} onChange={() => setToUnit('gram')} />
            <label htmlFor="toGram" style={{ fontSize: '16px', marginLeft: '5px', color: 'white' }}>Gram</label>
          </div>
        </div>
      </div>
